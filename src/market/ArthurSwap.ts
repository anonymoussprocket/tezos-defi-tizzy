...
export class ArthurSwap extends BaseSwap {
    ...
    constructBuyOperation(sourceAddress: string, size: string, notional: string, options?: any, counter: number = 0, fee?: OperationFee) {
        const expiration = options['expiration'] as Date;
        const params = `{ "prim": "Pair", "args": [ { "string": "${sourceAddress}" }, { "prim": "Pair", "args": [ { "int": "${size}" }, { "string":"${expiration.toISOString()}" } ] } ] }`;
    ...
    matchBuyOperation(operationGroup: any[]): OperationMatch {
        ...
        for (let i = 0; i < operationGroup.length; i++) {
            ...
            if (o['destination'] === this.poolAddress && o['parameters']['entrypoint'] === 'xtzToToken') {
                ...
                const tokenMinimum = JSONPath({ path: '$.args[1].args[0].int', json: params })[0];
    ...
    matchSellOperation(operationGroup: any[]): OperationMatch {
        ...
        for (let i = 0; i < operationGroup.length; i++) {
            ...
            if (o['destination'] === this.poolAddress && o['parameters']['entrypoint'] === 'tokenToXtz') {
                ...
                let tokenBalance = JSONPath({ path: '$.args[1].args[0].int', json: params })[0];
                if (tokenBalance === undefined) {
                    tokenBalance = JSONPath({ path: '$.args[1].int', json: params })[0];
                }

                let coinMinimum = JSONPath({ path: '$.args[1].args[1].args[0].int', json: params })[0];
                if (coinMinimum === undefined) {
                    coinMinimum = JSONPath({ path: '$.args[2].int', json: params })[0];
                }
