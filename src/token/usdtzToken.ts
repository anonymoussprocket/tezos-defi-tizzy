...
export class usdtzToken implements BaseToken {
    readonly tokenAddress = 'KT1LN4LPSqTMS7Sd2CJw4bbDGRkMv2t68Fy9';
    readonly tokenType = 'tzip7';
    readonly tokenSymbol = 'USDtz';
    readonly tokenDecimals = 6;
    readonly tokenLedgerMap = 36;
    readonly tokenLedgerKeyType = 'address';
    readonly tokenLedgerPath = '$.args[0].int';
    readonly tokenApprovalMap = 36;
    ...
    async getApproval(server: string, sourceAddress: string, destinationAddress: string, mapIndex: number = this.tokenLedgerMap): Promise<string> {
        ...
        try {
            ...
            JSONPath({ path: '$.args[1][*].args', json: mapResult }).forEach(v => allowances[v[0]['string']] = v[1]['int']);
